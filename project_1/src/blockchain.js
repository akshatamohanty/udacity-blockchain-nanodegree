...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                block.time = Date.now()
                block.height = height
                block.previousBlockHash = chain.length ? chain[chain.length - 1].hash : null
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                const timeInSeconds = message.split(':')[1]
                const currentTimeInSeconds = parseInt(new Date().getTime().toString().slice(0, -3))
                const timeDifferenceInSeconds = (currentTimeInSeconds - timeInSeconds)

                if (timeDifferenceInSeconds > FIVE_MINUTES) {
                    throw new Error('block expired')
                }

                const isVerified = bitcoinMessage.verify(message, address, signature)
                if (isVerified) {
                    const blockData = { star, owner: address }
